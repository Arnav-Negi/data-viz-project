...
getData().then((data) => {
    ...
    // condense all contries's deaths into one world death count
    const diseaseData = [];
    data.forEach((row) => {
        const date = new Date(row.Year, 0, 1);
    ...
    const graph = Streamgraph(diseaseData, svg, {
        x: d => d.date,
        y: d => d.deaths,
        z: d => d.name,
        yLabel: 'Deaths',
    ...
    document.getElementById('chart').appendChild(graph);
