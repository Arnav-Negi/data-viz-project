...
export default function AgeAreaChart() {
    const ageRanges = ['>70', '50-69', '15-49', '5-14', '<5']
    const [dataArray, setDataArray] = useState([]);

    function build(country) {
        document.getElementById('age').append(StackedAreaChart(dataArray.filter((d) => d.country === country), {
            x: (d) => d.year,
            y: (d) => d.deaths,
            z: (d) => d.age,
    ...
    useEffect(() => {
        const fetchData = async () => {
            let tempArray = [];
            await d3.csv('https://raw.githubusercontent.com/Arnav-Negi/data-viz-project/main/data/number-of-deaths-by-age-group.csv', (d) => {
